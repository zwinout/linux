...
static int omen_gpu_thermal_profile_get(bool *ctgp_enable,
					    bool *ppab_enable,
					    u8 *dstate,
					    u8 *gpu_slowdown_temp)
{
	struct omen_gpu_power_modes gpu_power_modes;
	int ret;
					    ...
					    u8 dstate)
{
	...
	ret = omen_gpu_thermal_profile_get(&current_ctgp_state,
					       &current_ppab_state,
					       &current_dstate,
...
static int omen_thermal_profile_set(int mode)
{
	...
	if (mode == HP_OMEN_V1_THERMAL_PROFILE_PERFORMANCE ||
	    mode == HP_OMEN_V0_THERMAL_PROFILE_PERFORMANCE) {
		omen_gpu_thermal_profile_set(true, true, 1);
	} else {
		omen_gpu_thermal_profile_set(false, true, 1);
	}
